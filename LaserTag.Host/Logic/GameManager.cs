...
namespace TekHub.Host.Logic
{
    public partial class GameManager : ObservableObject
    {
        ...
        public async Task EndMatch()
        {
            ...
            await GameService.AddNewMatchAsync(Match);
            await GameService.AddRoundToMatchAsync(Match);
            await GameService.AddShootLog(ShootLogs);
            await GameService.AddHitLog(HitLogs);
            Match = new();
        ...
        {
            await InitDataAttribute();  // Wait for this to finish before calling InitDataUpgrade
            InitDataUpgrade();
            InitDataGameConfig();
        }

        public async Task InitDataAttribute()
        {
            // Step 1: Load the Config data (simulated from the seeded database)
            var configData = await GameService.GetAllConfigsAsync();
            ...
            var newGameAttributes = await GameService.GetAllAttributeAsync();
